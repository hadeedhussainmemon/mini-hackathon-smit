...
if (!apiKey) {
  console.error('GEMINI_API_KEY not set in environment. Server-side Gemini calls will fail.')
}
...
export const generatePitch = async (ideaData) => {
  try {
    ...
    if (!apiKey) {
      throw new Error('GEMINI_API_KEY not set. Set the environment variable on the server (GEMINI_API_KEY).')
    }
    ...
    try {
      ...
      console.error('Failed to parse Gemini output as JSON. Raw output:', cleanedText)
